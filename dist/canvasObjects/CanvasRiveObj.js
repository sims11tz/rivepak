...
export var RIVE_DEBUG_IN_EDITOR;
(function (RIVE_DEBUG_IN_EDITOR) {
    RIVE_DEBUG_IN_EDITOR["TRUE"] = "TRUE";
    RIVE_DEBUG_IN_EDITOR["FALSE"] = "FALSE";
})(RIVE_DEBUG_IN_EDITOR || (RIVE_DEBUG_IN_EDITOR = {}));
...
export class CanvasRiveObj extends BaseCanvasObj {
    ...
    get layoutFit() { return this._layoutFit; }
    set layoutFit(value) { this._layoutFit = value; }
    get layoutAlignment() { return this._layoutAlignment; }
    set layoutAlignment(value) { this._layoutAlignment = value; }
    /**
     * Set both layout fit and alignment in one call
     * @example obj.SetLayout(obj.Rive.Fit.cover, obj.Rive.Alignment.center)
     */
    SetLayout(fit, alignment) {
        this._layoutFit = fit;
    ...
    constructor(riveDef, artboard) {
        ...
        this._layoutFit = null;
        this._layoutAlignment = null;
    ...
    */
        ...
        this._layoutFit = (_c = riveDef.layoutFit) !== null && _c !== void 0 ? _c : this._riveInstance.Fit.contain;
        this._layoutAlignment = (_d = riveDef.layoutAlignment) !== null && _d !== void 0 ? _d : this._riveInstance.Alignment.topLeft;
    }
    ...
    Update(time, frameCount, onceSecond, onceMinute) {
        ...
        if (!this._disposed && this.visible) {
            ...
            if (this._resolutionScale !== -1) {
                ...
                // Apply visual offset to shift rendered artwork relative to x,y position
                const visualOffsetX = this._visualXOffset * this._resolutionScale;
                const visualOffsetY = this._visualYOffset * this._resolutionScale;
                this._objBoundsReuse.minX = Math.round((this._transformedX + visualOffsetX) * dpr);
                this._objBoundsReuse.minY = Math.round((this._transformedY + visualOffsetY) * dpr);
                this._objBoundsReuse.maxX = Math.round((this._transformedX + visualOffsetX + (scaledWidth * this._resolutionScale)) * dpr);
                this._objBoundsReuse.maxY = Math.round((this._transformedY + visualOffsetY + (scaledHeight * this._resolutionScale)) * dpr);
            }
            else {
                // Apply visual offset to shift rendered artwork relative to x,y position
                this._objBoundsReuse.minX = Math.round((this.x + this._visualXOffset) * dpr);
                this._objBoundsReuse.minY = Math.round((this.y + this._visualYOffset) * dpr);
                this._objBoundsReuse.maxX = Math.round((this.x + this._visualXOffset + scaledWidth) * dpr);
                this._objBoundsReuse.maxY = Math.round((this.y + this._visualYOffset + scaledHeight) * dpr);
            }
            this.Renderer.save();
            ...
            if (this.defObj.scaleMode === OBJECT_SCALE_MODE.STRETCH) {
                ...
                if (this.defObj.scaleMode === OBJECT_SCALE_MODE.STRETCH) {
                    ...
                    if (daveDebug) {
                        if (onceSecond)
                            console.log(' !--!(' + this.label + ') === OBJECT_SCALE_ALIGN.TOP_CENTER)!--!       pixi.w=' + PixiController.get().PixiAbove.view.width);
                        if (onceSecond)
                            console.log(' !--!(' + this.label + ') === OBJECT_SCALE_ALIGN.TOP_CENTER)!--! offsetNumber=' + offsetNumber);
                    }
                ...
                this.Renderer.align(this._layoutFit, this._layoutAlignment, objBoundsReuse, this.artboard.bounds);
            }
            else {
                if (daveDebug && onceSecond)
                    console.log(' !--!(' + this.label + ') === no stretch -- default whatever that is.. ');
                // Use configured layout properties
                this.Renderer.align(this._layoutFit, this._layoutAlignment, this._objBoundsReuse, this.artboard.bounds);
            }
            if (this._interactiveGraphics) {
                ...
                const debugDrawThisHere = (_a = this._debugRive) !== null && _a !== void 0 ? _a : false;
